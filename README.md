# job-search-companion

一个面向中国大陆求职场景的求职陪跑 Skill。

它的重点不是“把简历写得更会吹”，而是帮候选人把混乱材料变成可投递、可追问、可复盘的一套求职系统：先收集事实，再拆证据，再针对 JD 做取舍，最后输出简历、PDF、作品集、投递话术、面试故事和谈薪清单。

适用范围不限软件开发，也不限互联网岗位。应届生、转行候选人、门店/零售/销售/行政/内容/运营/设计/教育/服务业等经历，都可以先进入素材库，再按目标岗位重新编排。

## 它解决什么问题

很多求职材料的问题不是“文案不高级”，而是：

- 经历很多，但不知道哪段该写进简历，哪段该放作品集。
- 用户给的信息不完整，AI 容易开始脑补数字和成果。
- JD 看起来都差不多，实际硬门槛、隐性要求和红旗词没拆开。
- 国内投递不是一份简历走天下，还要处理 BOSS、微信、内推、平台在线简历和附件 PDF。
- 面试会追问空窗、转行、短经历、第一学历、薪资流水、五险一金和背调。
- 视觉简历好看但 ATS 不友好，或 PDF 生成了却没做版式 QA。

这个 Skill 的原则是：**真实、克制、能落地；可以包装表达，不能伪造经历。**

## PDF 模板展示

同一份结构化简历数据，可以按渠道切换不同版式。ATS 版负责解析，视觉版负责人工阅读，作品/案例页负责补充证据。

![PDF 简历版式库](docs/images/pdf-layout-gallery.png)

主题色可以通过 `--theme` 或自定义 `accent` 变量切换。正文结构不变，颜色只服务于岗位气质和阅读层级。

![PDF 主题色一键切换](docs/images/pdf-theme-gallery.png)

> 截图使用虚构候选人数据，仅用于展示模板效果。

## 能力地图

| 模块 | 解决的问题 | 典型产物 |
| --- | --- | --- |
| 启动与信息采集 | 用户材料很乱、不知道怎么开始 | 3 个以内启动问题、已有材料清单、待补材料 |
| 个人素材库 | 把经历、证书、作品、校园、兼职、项目沉淀下来 | 素材索引、经历卡片、可迁移能力标签 |
| 证据模型 | 判断什么能写、什么要降级、什么不能公开 | 强/中/弱证据、披露安全等级、量化补采表 |
| JD 拆解 | 从岗位描述里拆出硬门槛、任务、隐含要求和红旗词 | JD 匹配矩阵、缺口清单、投递建议 |
| 简历编排 | 多段经历该怎么取舍，教育/项目/工作谁前置 | 2-3 套编排方案、经历取舍清单、用户确认门 |
| 材料生成 | 输出简历、作品集、投递话术和面试材料 | ATS 简历、视觉 PDF、作品集结构、STAR 故事 |
| 国内投递 | 适配 BOSS、微信、内推、智联、猎聘等渠道 | 30-100 字开场、跟进话术、内推包 |
| 面试准备 | 处理 HR 面、业务面、主管面、群面、转行追问 | 自我介绍、项目深挖、公司理解、反问问题 |
| 谈薪与 Offer | 拆税前、N 薪、五险一金、试用期、背调 | 薪资结构表、Offer 对比、风险清单 |
| 求职复盘 | 投递没反馈、连续失败、空窗焦虑时调整节奏 | 投递 tracker、面试复盘、情绪与行动节奏 |

## 一条典型工作流

```text
接收资料
-> 整理候选人素材
-> 拆证据与风险
-> 明确目标岗位和限制
-> 拆 JD 与公司背景
-> 映射岗位要求和个人素材
-> 先确认简历重点与经历取舍
-> 生成简历 / 作品集 / 投递话术
-> 评分、补缺口、做 PDF QA
-> 进入投递、面试、谈薪、Offer 复盘
```

如果只是一个小任务，比如“帮我改一句 BOSS 打招呼”，Skill 会走轻量路径；不会强行跑完整流程。

如果是完整简历或 PDF，Skill 会先做确认门：

- 这版简历优先突出什么。
- 哪些经历进入正文，哪些弱化或移入作品集。
- 是否需要 ATS 版、视觉版、照片版、zc 风格版。
- 哪些数字、截图、公司名、学校名和公开边界仍未确认。

## 使用示例

迷茫启动：

```text
我想找工作，但简历很乱，不知道怎么开始。
```

带材料启动：

```text
这是我的旧简历、作品集链接和一个 JD。先帮我判断这版该怎么改。
```

只做局部任务：

```text
帮我把这段门店运营经历写成简历 bullet，但不要夸大。
```

针对 JD 定制：

```text
我想投这个岗位，帮我拆 JD、判断匹配度，再告诉我简历应该突出哪些经历。
```

PDF 输出：

```text
把这版简历做成 ATS 版和一个清新绿视觉版 PDF，先给我版式方案。
```

## 安装

把本仓库放到 Codex Skills 目录：

```bash
mkdir -p ~/.codex/skills
git clone https://github.com/leoli001031-blip/job-search-companion-skill.git ~/.codex/skills/job-search-companion
```

如果你的 Agent 支持显式指定 Skills 目录：

```bash
--skills-dir "$HOME/.codex/skills"
```

触发方式可以很自然：

```text
用 job-search-companion 帮我整理求职材料。
```

## PDF 导出器

内置轻量导出器：

```text
scripts/render-resume-pdf.mjs
```

示例：

```bash
node scripts/render-resume-pdf.mjs \
  --input examples/resume-pdf-data.example.json \
  --out /tmp/resume-out \
  --template cn-sidebar \
  --theme green
```

常用模板：

| 模板 | 场景 |
| --- | --- |
| `ats-clean` | 平台解析、官网网申、批量投递 |
| `cn-single-polished` | 单栏精致版，适合大多数人工阅读 |
| `cn-sidebar` | 左侧栏版，适合信息分区和微信转发 |
| `right-rail-modern` | 右侧信息栏，清爽稳重 |
| `compact-two-column` | 内容较多，需要一页压缩 |
| `top-band-compact` | 顶部信息带，传统正式 |
| `zc-sidebar-visual` | zc 视觉风格，必须显式触发 |

常用主题：

```text
blue / green / black / orange / navy / teal / cyan / indigo / wine / slate / zc
```

导出器会做基础 QA：

- 模板占位符是否残留。
- `[待确认]`、`[待补充]` 是否误进正式版。
- 评分、风险提示、通篇复核等内部内容是否污染简历。
- 多段经历/项目是否超出模板容量，被静默省略。
- PDF/PNG 是否真实生成，而不是只生成 HTML。

## 国内求职特化

这个 Skill 默认按国内求职语境工作，覆盖：

- BOSS 直聘、智联、猎聘、前程无忧、拉勾、实习僧、官网网申。
- 微信发简历、加 HR、面试后跟进、微信群/熟人内推。
- 在线简历和附件简历的双轨检查。
- 国企/传统企业/校招/实习场景下的教育、证书、照片、政治面貌、籍贯/户籍判断。
- 年龄、婚育、空窗期、第一学历、非相关专业、户籍、频繁跳槽等敏感问题。
- 税前税后、N 薪、绩效、提成、五险一金、试用期、薪资流水、竞业、服务期、背调。
- 秋招/春招、群面、测评、笔试、三方协议、违约风险。

## 证据与隐私边界

Skill 会区分两件事：

```text
证据强不强
能不能公开
```

例如内部表格、微信群截图、薪资流水、客户名单可能能证明经历，但不适合直接进入简历或公开作品集。

默认处理方式：

- 简历正文只写可读摘要。
- 截图、表格、文档、聊天记录优先做脱敏作品集或案例页。
- 用户标注“只供参考 / 不要公开 / 不要写”的材料，不能进入投递版。
- 没有确认的数字不写成真实成果。
- “协助/参与/使用过”不能升级成“主导/负责/熟练”。

## 输出风格

每次正式使用 Skill 输出求职产物时，回复首行会带运行标记：

```text
【Skill运行】job-search-companion｜阶段：...｜依据：...｜本轮产物：...
```

这可以帮助测试和多轮对话判断：当前是否真的走了 Skill，而不是退化成普通聊天。

## 目录结构

```text
.
├── SKILL.md
├── agents/
├── assets/
│   └── resume-pdf-templates/
├── docs/
│   ├── demo/
│   └── images/
├── examples/
├── references/
└── scripts/
```

重要 reference：

- `references/evidence-model.md`
- `references/materials.md`
- `references/jd-tailoring.md`
- `references/output-evaluation.md`
- `references/pdf-export.md`
- `references/domestic-delivery-channels.md`
- `references/domestic-sensitive-questions.md`
- `references/domestic-salary-negotiation.md`
- `references/tracking.md`

## 限制

- 不能替用户编造经历、学历、证书、项目、数据或 Offer。
- 不能替代法律、税务、心理咨询或劳动仲裁专业意见。
- 公司调研必须区分事实、推断、匿名信息和信息不足。
- 视觉 PDF 不能替代 ATS 单栏版；正式投递前仍应打开预览做人工检查。

## License

暂未指定开源许可证。未经作者明确授权，不建议直接用于商业分发。
